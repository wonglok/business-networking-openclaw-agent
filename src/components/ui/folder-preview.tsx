...
export type FolderVariant = 'devi' | 'rudras' | 'ardra' | 'shakti' | 'kubera' | 'hari' | 'ravi' | 'durga' | 'nandi'
  ...
  return (
    <div className='relative'>
      ...
      <div className='relative cursor-pointer aspect-[20/16]' style={{ perspective: '800px' }}>
        ...
        {/* Cover */}
        <motion.div
          className={cn('relative transition-colors duration-150', isHovered ? 'text-gray-600' : colors.cover)}
        >
          <FolderCoverIcon />
      ...
      {label && <h3 className={cn('mt-3 font-medium text-center', sizes.caption, colors.caption)}>{label}</h3>}
    </div>
  )
}
...
const RudrasFolder: React.FC<{
  ...
  const positions: any = createCircularPositions(images.length, 80)
    ...
    <div className='relative'>
      ...
      >
        <motion.div
          ...
          initial={{ scale: 1 }}
          animate={isHovered ? { opacity: [1, 0], scale: [1, 6] } : { opacity: 0, scale: 1 }}
    ...
    >
      <div className='relative cursor-pointer'>
        ...
        <div className={cn('absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2', sizes.thumb)}>
          {images.map((img, i) => (
            <motion.img
              ...
              animate={
                isHovered ? { opacity: 1, rotate: -10 * (images.length - i - 1) - 15 } : { opacity: 0, rotate: 0 }
              }
              transition={{
                duration: 0.5,
                delay: i * 0.08,
        ...
        <div className={cn('relative', colors.cover)}>
          <FolderCoverIcon />
          <div
            className={cn('absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 mt-1', sizes.deco, colors.deco)}
          >
            <PadlockIcon />
          </div>
        </div>
...
const HariFolder: React.FC<{
  ...
  const positions: any = createCircularPositions(images.length, 120)
  ...
  return (
    <div className='relative'>
      ...
      <motion.div
        ...
        animate={isHovered ? { opacity: [1, 0], scale: [1, 15] } : { opacity: 0, scale: 1 }}
      ...
      <motion.div
        ...
        animate={isHovered ? { y: -20, scaleX: 0.9, scaleY: 0.9 } : { y: 0, scaleX: 1, scaleY: 1 }}
        transition={{
          duration: 0.8,
          type: 'spring',
          stiffness: 200,
  ...
  return (
    <div className='relative'>
      ...
      <motion.div
        ...
        initial={{ opacity: 0, scale: 1 }}
        animate={!isHovered ? { opacity: [1, 0], scale: [1, 5] } : { opacity: 0, scale: 1 }}
      ...
      <div className='relative cursor-pointer' style={{ perspective: '800px' }}>
        ...
        <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[75px] h-[65px]'>
          {orderedImages.map((img, i) => {
            ...
            const rotate =
              -rotateInterval * Math.floor(c / 2) + rotateInterval * i + (c % 2 === 0 ? rotateInterval / 2 : 0)
            ...
            return (
              <motion.img
                ...
                animate={
                  isHovered ? { opacity: 1, y: -70, x, rotate } : { opacity: 0, x: 0, y: 0, rotate: 0, scale: 0.5 }
                }
                transition={{
      ...
      >
        {files.slice(0, 6).map((file, i) => (
          <div key={i} className='text-center'>
            <FileIcon className={cn('w-5 h-5 mx-auto', fileColorMap[file.type || 'default'])} />
            <span className='text-[8px] text-gray-600 block mt-0.5 truncate max-w-[35px]'>{file.name}</span>
          </div>
        ))}
      </motion.div>
...
export const FolderPreview = React.forwardRef<HTMLDivElement, FolderPreviewProps>(
  ({ variant = 'devi', images = [], files = [], label, size = 'md', className, onClick }, ref) => {
