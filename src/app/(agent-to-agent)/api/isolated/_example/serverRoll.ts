...
export const serverRoll = async ({ files = [] }: any) => {
    ...
    let bundler = rollup({
        ...
        plugins: [
            {
                name: 'loklok-runner',
                async resolveId(moduleName: string, parentBaseURL: string) {
                    //
                    console.log('resolving module: ', moduleName, 'wanted by', parentBaseURL);
                ...
                async load(id: string) {
                    ...
                    let file = files.find((e: any) => removeTSJS(`${CodePrefix}${e.path}`) === removeTSJS(id));
    ...
    try {
        ...
        let finalOutput = gen.output.map((res: any) => {
            return {
                ...res,
